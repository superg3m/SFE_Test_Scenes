...
void render() {
    ...
    /*
    GFX::Canvas2D::Begin(orthographic);
        float round = 0.5f; // 0 - 1
        GFX::Canvas2D::DrawRectangle(color, x, y, width, height, round);
        GFX::Canvas2D::DrawCircle(color, x, y, radius);

        const char* text = "Wow this works\nthis doesn't work does it?";
        GFX::Canvas2D::DrawText(app.font, text, 200, 200) // I don't need this anymore &app.text_shader);
    GFX::Canvas2D::End();
    */
}
